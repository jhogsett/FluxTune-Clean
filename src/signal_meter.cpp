...
void SignalMeter::init()
{
    ...
    _panel_led_accumulator = 0;
#ifndef NATIVE_BUILD
    // Initialize NeoPixel strip for all platforms
    if (!_led_strip) {
...
void SignalMeter::write_leds()
{
    ...
    if (_flashlight_mode) {
        ...
        // NeoPixel implementation for all platforms
        if (_led_strip) {
        ...
        if (_current_strength > 0) {
            ...
            display_strength = (int)(sqrt_value * 16.0f);
